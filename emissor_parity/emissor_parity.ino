...
void loop() {
  ...
  for (int j = 0; j < stringToSend.length(); j++) { // iterate over all characters of the string
    ...
    for (int i = 0; i < 9; i++) { // iterate over all 9 bits of the 
      if (i == 8) {
    ...
    delayMicroseconds(105); // wait for 10 milisecond before sending the next character

  }
  Serial.println("");
  delayMicroseconds(105); // wait for 1 second before sending the next character
}
