...
void loop() {
  ...
  for (int j = 0; j < stringToSend.length(); j++) { // iterate over all characters of the string
    ...
    Serial.println("");
    Serial.println("-- Bits enviados --");
    
    delay(1); // wait for 1 ms before transmitting the first bit
    for (int i = 0; i < 8; i++) { // iterate over all 8 bits of the payload
      if (bitRead(characterToSend, i) == 1) { // check if the i-th bit of the character is 1
        ...
        Serial.print("0");
         ...
         delay(1); // wait for 1 milisecond before transmitting the next bit
    }

    Serial.println("");
    Serial.println("-- CRC --");

    //API used to get the CRC
    crc.reset();
    crc.add(characterToSend);
    crc_bits = crc.getCRC();

    // Send the CRC8 
    for (int i = 0; i < 8; i++) { // iterate over all 8 bits of the crc
      if (bitRead(crc_bits, i) == 1) { // check if the i-th bit of the character is 1
        digitalWrite(outputPin, HIGH); // set the output pin to HIGH to transmit a 1
        Serial.print("1");
      } else if(bitRead(crc_bits, i) == 0) {
        digitalWrite(outputPin, LOW); // set the output pin to LOW to transmit a 0
        Serial.print("0");
      }
         delay(1); // wait for 1 milisecond before transmitting the next bit
